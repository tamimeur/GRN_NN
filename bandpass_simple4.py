    ...
    for gene in species:
        if gene in design:
            ...
            if design[inx-1] == 'p1':
                ...
                #p1 const exp strongly, rep G1
            elif design[inx-1] == 'p2':
                eqn.append(a2/(1+k2*X[2]) - deg[spec]*X[spec])
                #p2 const exp med, rep by G2
            elif design[inx-1] == 'p3':
                eqn.append((a3*s)/(1+s) - deg[spec]*X[spec])
                #p3 induced strongly by signal s
            elif design[inx-1] == 'p4':
                eqn.append(a1/(1) - deg[spec]*X[spec])
                #p4 const exp strongly
            elif design[inx-1] == 'p5':
                eqn.append(a4/(1) - deg[spec]*X[spec])
                #p4 const exp weak
        else:
            eqn.append(0)
